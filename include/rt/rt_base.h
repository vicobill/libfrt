 ...
 *  PRId8, PRId16, PRId32, etc, SCNd8, SCNd16, SCNd32
 *	PRIiSIZE,PRIuSIZE, PRIxSIZE
 *	FUX_SUCC,FUX_FAIL
 *	FUX_BYTE,FUX_KILOBYTE,FUX_MEGABYTE,FUX_GIGABYTE,FUX_TERABYTE,FUX_PETABYTE,FUX_EXABYTE
 */
#ifndef FUX_RT_TYPES_H_INCLUDED
#define FUX_RT_TYPES_H_INCLUDED

#include "rt_platform.h"
#include "rt_compiler.h"
#include "rt_compilertraits.h"
...
#if defined(FUX_COMPILER_GNUC) || defined(FUX_COMPILER_MSVC) || defined(FUX_WCHAR_T_NON_NATIVE) || defined(FUX_PLATFORM_KETTLE)
...
// is defined.
#if defined(FUX_COMPILER_CLANG) && defined(FUX_PLATFORM_APPLE)
    #include <stddef.h>
#endif
...
// would have put this into the compilertraits header.
#if !defined(FUX_COMPILER_HAS_INTTYPES) && (!defined(_MSC_VER) || (_MSC_VER > 1500)) \
    && (defined(FUX_COMPILER_IS_C99) || defined(INT8_MIN) || defined(FUX_COMPILER_HAS_C99_TYPES) || defined(_SN_STDINT_H))
    #define FUX_COMPILER_HAS_INTTYPES
#endif

#ifdef FUX_COMPILER_HAS_INTTYPES // If the compiler supports inttypes...
    // ------------------------------------------------------------------------
    ...
    #if !defined(FUX_COMPILER_MSVC) || (defined(FUX_COMPILER_MSVC) && FUX_COMPILER_VERSION >= 1800)
    ...
    #if defined(__MINGW32__) || (defined(FUX_PLATFORM_ANDROID) && !(defined(FUX_ANDROID_SDK_LEVEL) && FUX_ANDROID_SDK_LEVEL >= 21))
    ...
    #if   defined(FUX_COMPILER_MSVC) || defined(FUX_COMPILER_BORLAND)
...
// type sizes
...
#if defined(FUX_COMPILER_HAS_INTTYPES) && (!defined(FUX_COMPILER_MSVC) || (defined(FUX_COMPILER_MSVC) && FUX_COMPILER_VERSION >= 1800))
    #define FUX_COMPILER_HAS_C99_FORMAT_MACROS
#endif

#ifndef FUX_COMPILER_HAS_C99_FORMAT_MACROS
    // ------------------------------------------------------------------------
    ...
    #if defined(FUX_COMPILER_MSVC) || defined(FUX_COMPILER_BORLAND) // VC++ 7.1+ understands long long as a data type but doesn't accept %ll as a printf specifier.
        #define FUX_PRI_64_LENGTH_SPECIFIER "I64"
        #define FUX_SCN_64_LENGTH_SPECIFIER "I64"
    #else
        #define FUX_PRI_64_LENGTH_SPECIFIER "ll"
        #define FUX_SCN_64_LENGTH_SPECIFIER "ll"
    ...
    #if defined(FUX_COMPILER_IS_C99) || defined(FUX_COMPILER_GNUC)
        #define PRId8     "hhd"
        #define PRIi8     "hhi"
        #define PRIo8     "hho"
        #define PRIu8     "hhu"
    ...
    #define PRId64        FUX_PRI_64_LENGTH_SPECIFIER "d"
    #define PRIi64        FUX_PRI_64_LENGTH_SPECIFIER "i"
    #define PRIo64        FUX_PRI_64_LENGTH_SPECIFIER "o"
    #define PRIu64        FUX_PRI_64_LENGTH_SPECIFIER "u"
    #define PRIx64        FUX_PRI_64_LENGTH_SPECIFIER "x"
    #define PRIX64        FUX_PRI_64_LENGTH_SPECIFIER "X"
    
    #if (FUX_PLATFORM_PTR_SIZE == 4)
    ...
    #define SCNd64        FUX_SCN_64_LENGTH_SPECIFIER "d"
    #define SCNi64        FUX_SCN_64_LENGTH_SPECIFIER "i"
    #define SCNo64        FUX_SCN_64_LENGTH_SPECIFIER "o"
    #define SCNu64        FUX_SCN_64_LENGTH_SPECIFIER "u"
    #define SCNx64        FUX_SCN_64_LENGTH_SPECIFIER "x"
    
    #if defined(FUX_COMPILER_MSVC) && (FUX_COMPILER_VERSION >= 1900)
        #define SCNdPTR       PRIdPTR
        #define SCNiPTR       PRIiPTR
        #define SCNoPTR       PRIoPTR
        #define SCNuPTR       PRIuPTR
        #define SCNxPTR       PRIxPTR
    #elif (FUX_PLATFORM_PTR_SIZE == 4)
...
//
    ...
    #if defined(FUX_COMPILER_MSVC) || (defined(FUX_COMPILER_INTEL) && defined(FUX_PLATFORM_WINDOWS)) || defined(FUX_COMPILER_BORLAND)
        #if defined(__cplusplus)
            typedef bool bool8_t;
        #else
            typedef int8_t bool8_t;
        #endif
    #else // FUX_COMPILER_GNUC generally uses 4 bytes per bool.
        typedef int8_t bool8_t;
    #endif
#endif
...
//
#if !defined(_INTPTR_T_DEFINED) && !defined(_intptr_t_defined) && !defined(FUX_COMPILER_HAS_C99_TYPES)
    #if (FUX_PLATFORM_PTR_SIZE == 4)
...
#if !defined(_UINTPTR_T_DEFINED) && !defined(_uintptr_t_defined) && !defined(FUX_COMPILER_HAS_C99_TYPES)
    #if (FUX_PLATFORM_PTR_SIZE == 4)
...
#if !defined(FUX_COMPILER_HAS_INTTYPES)
...
//
    ...
    // so for all compilers, as size_t may be based on int, long, or long long.
        ...
        #if defined(_MSC_VER) && (FUX_PLATFORM_PTR_SIZE == 8)
...
//
#if defined(FUX_COMPILER_MSVC) || defined(FUX_COMPILER_BORLAND)
    #if defined(FUX_WCHAR_T_NON_NATIVE)
...
// _CHAR16T and let EABase define char16_t and FUX_CHAR16.
//
// GCC defines char16_t and char32_t in the C compiler in -std=gnu99 mode,
// as __CHAR16type_ and __CHAR32type_, and for the C++ compiler
// in -std=c++0x and -std=gnu++0x modes, as char16_t and char32_t too.
//
// The FUX_WCHAR_UNIQUE symbol is defined to 1 if wchar_t is distinct from
...
// takes a unicode string.
...
#if !defined(FUX_CHAR16_NATIVE)
    // To do: Change this to be based on FUX_COMPILER_NO_NEW_CHARACTER_TYPES.
    #if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
        #define FUX_CHAR16_NATIVE 1
    #elif defined(FUX_COMPILER_CLANG) && defined(FUX_COMPILER_CPP11_ENABLED)
        #if __has_feature(cxx_unicode_literals)
            #define FUX_CHAR16_NATIVE 1
        #elif (FUX_COMPILER_VERSION >= 300) && !(defined(FUX_PLATFORM_IPHONE) || defined(FUX_PLATFORM_OSX))
            #define FUX_CHAR16_NATIVE 1
        #elif defined(FUX_PLATFORM_APPLE)
            #define FUX_CHAR16_NATIVE 1
        #else
            #define FUX_CHAR16_NATIVE 0
        #endif
    #elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR16type_) && defined(FUX_COMPILER_CPP11_ENABLED)// EDG 4.4+.
        #define FUX_CHAR16_NATIVE 1
    #elif defined(FUX_COMPILER_GNUC) && (FUX_COMPILER_VERSION >= 4004) && !defined(FUX_COMPILER_EDG) && (defined(FUX_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__)) // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
        #define FUX_CHAR16_NATIVE 1
    #else
        #define FUX_CHAR16_NATIVE 0
    #endif
...
#if !defined(FUX_CHAR32_NATIVE)                    // Microsoft currently ties char32_t language support to char16_t language support. So we use CHAR16_T here.
    // To do: Change this to be based on FUX_COMPILER_NO_NEW_CHARACTER_TYPES.
    #if defined(_MSC_VER) && (_MSC_VER >= 1600) && defined(_HAS_CHAR16_T_LANGUAGE_SUPPORT) && _HAS_CHAR16_T_LANGUAGE_SUPPORT // VS2010+
        #define FUX_CHAR32_NATIVE 1
    #elif defined(FUX_COMPILER_CLANG) && defined(FUX_COMPILER_CPP11_ENABLED)
        #if __has_feature(cxx_unicode_literals)
            #define FUX_CHAR32_NATIVE 1
        #elif (FUX_COMPILER_VERSION >= 300) && !(defined(FUX_PLATFORM_IPHONE) || defined(FUX_PLATFORM_OSX))
            ...
            #define FUX_CHAR32_NATIVE 1
        #else
            #define FUX_CHAR32_NATIVE 0
        #endif
    #elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 404) && defined(__CHAR32type_) && defined(FUX_COMPILER_CPP11_ENABLED)// EDG 4.4+.
        #define FUX_CHAR32_NATIVE 1
    #elif defined(FUX_COMPILER_GNUC) && (FUX_COMPILER_VERSION >= 4004) && !defined(FUX_COMPILER_EDG) && (defined(FUX_COMPILER_CPP11_ENABLED) || defined(__STDC_VERSION__)) // g++ (C++ compiler) 4.4+ with -std=c++0x or gcc (C compiler) 4.4+ with -std=gnu99
        #define FUX_CHAR32_NATIVE 1
    #else
...
#if FUX_CHAR16_NATIVE || FUX_CHAR32_NATIVE
    #define FUX_WCHAR_UNIQUE 1
#else
    #define FUX_WCHAR_UNIQUE 0
#endif
    ...
    #define CHAR8_T_DEFINED
        ...
        // In GCC C, __CHAR16type_ and __CHAR32type_ are defined instead, and we must define char16_t and char32_t from these.
        #if defined(__GNUC__) && !defined(__GXX_EXPERIMENTAL_CXX0X__) && defined(__CHAR16type_) // If using GCC and compiling in C...
            typedef __CHAR16type_ char16_t;
            typedef __CHAR32type_ char32_t;
        #endif
    #elif (FUX_WCHAR_SIZE == 2)
...
//
#define FUX_LIMITS_DIGITS_S(T)  ((sizeof(T) * 8) - 1)
#define FUX_LIMITS_DIGITS_U(T)  ((sizeof(T) * 8))
#define FUX_LIMITS_DIGITS(T)    ((FUX_LIMITS_IS_SIGNED(T) ? FUX_LIMITS_DIGITS_S(T) : FUX_LIMITS_DIGITS_U(T)))
#define FUX_LIMITS_IS_SIGNED(T) ((T)(-1) < 0)
#define FUX_LIMITS_MIN_S(T)     ((T)((T)1 << FUX_LIMITS_DIGITS_S(T)))
#define FUX_LIMITS_MIN_U(T)     ((T)0)
#define FUX_LIMITS_MIN(T)       ((FUX_LIMITS_IS_SIGNED(T) ? FUX_LIMITS_MIN_S(T) : FUX_LIMITS_MIN_U(T)))
#define FUX_LIMITS_MAX_S(T)     ((T)(((((T)1 << (FUX_LIMITS_DIGITS(T) - 1)) - 1) << 1) + 1))
#define FUX_LIMITS_MAX_U(T)     ((T)~(T)0)
#define FUX_LIMITS_MAX(T)       ((FUX_LIMITS_IS_SIGNED(T) ? FUX_LIMITS_MAX_S(T) : FUX_LIMITS_MAX_U(T)))

#if !defined(CHAR8_MIN)
    #define CHAR8_MIN FUX_LIMITS_MIN(char8_t)
#endif

#if !defined(CHAR8_MAX)
    #define CHAR8_MAX FUX_LIMITS_MAX(char8_t)
#endif

#if !defined(CHAR16_MIN)
    #define CHAR16_MIN FUX_LIMITS_MIN(char16_t)
#endif

#if !defined(CHAR16_MAX)
    #define CHAR16_MAX FUX_LIMITS_MAX(char16_t)
#endif

#if !defined(CHAR32_MIN)
    #define CHAR32_MIN FUX_LIMITS_MIN(char32_t)
#endif

#if !defined(CHAR32_MAX)
    #define CHAR32_MAX FUX_LIMITS_MAX(char32_t)
#endif



// FUX_CHAR8 / FUX_CHAR16 / FUX_CHAR32 / FUX_WCHAR
//
// Supports usage of portable string constants.
//
// Example usage:
//     const char16_t* str = FUX_CHAR16("Hello world");
//     const char32_t* str = FUX_CHAR32("Hello world");
//     const char16_t  c   = FUX_CHAR16('\x3001');
//     const char32_t  c   = FUX_CHAR32('\x3001');
//
#ifndef FUX_CHAR8
    #define FUX_CHAR8(s) s
#endif

#ifndef FUX_WCHAR
    #define FUX_WCHAR_(s) L ## s
    #define FUX_WCHAR(s)  FUX_WCHAR_(s)
#endif

#ifndef FUX_CHAR16
    #if FUX_CHAR16_NATIVE && !defined(_MSC_VER) // Microsoft doesn't support char16_t string literals.
        #define FUX_CHAR16_(s) u ## s
        #define FUX_CHAR16(s)  FUX_CHAR16_(s)
    #elif (FUX_WCHAR_SIZE == 2)
        #if defined(_MSC_VER) && (_MSC_VER >= 1900) && defined(__cplusplus) // VS2015 supports u"" string literals.
            #define FUX_CHAR16_(s) u ## s
            #define FUX_CHAR16(s)  FUX_CHAR16_(s)
        #else
            #define FUX_CHAR16_(s) L ## s
            #define FUX_CHAR16(s)  FUX_CHAR16_(s)
        #endif
    #else
        //#define FUX_CHAR16(s) // Impossible to implement efficiently.
    #endif
#endif

#ifndef FUX_CHAR32
    #if FUX_CHAR32_NATIVE && !defined(_MSC_VER) // Microsoft doesn't support char32_t string literals.
        #define FUX_CHAR32_(s) U ## s
        #define FUX_CHAR32(s)  FUX_CHAR32_(s)
    #elif (FUX_WCHAR_SIZE == 2)
        #if defined(_MSC_VER) && (_MSC_VER >= 1900) && defined(__cplusplus) // VS2015 supports u"" string literals.
            #define FUX_CHAR32_(s) U ## s
            #define FUX_CHAR32(s)  FUX_CHAR32_(s)
        #else
            //#define FUX_CHAR32(s) // Impossible to implement.
        #endif
    #elif (FUX_WCHAR_SIZE == 4)
        #define FUX_CHAR32_(s) L ## s
        #define FUX_CHAR32(s)  FUX_CHAR32_(s)
    #else
...
#pragma once
 ...
 // Example usage:
 //     int array[75];
 //     size_t arrayCount = FUX_ARRAY_COUNT(array); // arrayCount is 75.
 //
#if defined(FUX_COMPILER_NO_CONSTEXPR)
#ifndef FUX_ARRAY_COUNT
#define FUX_ARRAY_COUNT(x) (sizeof(x) / sizeof(x[0]))
#endif
#else
...
#define FUX_ARRAY_COUNT(x) (sizeof(sizeHelper(x)))
#endif
...
#elif !defined(__cplusplus) && defined(FUX_PLATFORM_ANDROID)
...
#elif defined(__EDG_VERSION__) && (__EDG_VERSION__ >= 401) && defined(FUX_COMPILER_CPP11_ENABLED)
	// static_assert is defined by the compiler.
...
#define FUX_STATIC_ASSERT_UNUSED_ATTRIBUTE        __attribute__((unused))
#else
#define FUX_STATIC_ASSERT_UNUSED_ATTRIBUTE
#endif
#define FUX_STATIC_ASSERT_TOKEN_PASTE(a,b)        a ## b
#define FUX_STATIC_ASSERT_CONCATENATE_HELPER(a,b) FUX_STATIC_ASSERT_TOKEN_PASTE(a,b)
...
#define static_assert(expression, description) \
	typedef char FUX_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert,__COUNTER__) [((expression) != 0) ? 1 : -1] \
		FUX_STATIC_ASSERT_UNUSED_ATTRIBUTE
#else
#define static_assert(expression, description) \ 
typedef char FUX_STATIC_ASSERT_CONCATENATE_HELPER(compileTimeAssert, __LINE__)[((expression) != 0) ? 1 : -1] \
FUX_STATIC_ASSERT_UNUSED_ATTRIBUTE
#endif
